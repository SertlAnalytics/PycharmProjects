...
from salesman_tutti.tutti_url_factory import TuttiUrlFactory, SalesmanSearchApi
import requests
import math
...
class SalesmanWebParser:
    ...
    def adjust_by_search_api(self, api: SalesmanSearchApi):
        ...
        self._url_factory.adjust_by_search_api(api)
    ...
    def fill_search_api_list_by_found_numbers(self, api_list: list, search_label_lists):
        for idx, api in enumerate(api_list):
            ...
            for search_label_list in search_label_lists:
                ...
                api.found_numbers += self.__get_found_number_for_product_category_url__(url)
    ...
    def __fill_category_value_found_number_dict__(
        ...
        for category_value in category_value_list:
            if self._search_api.category_value == '' or category_value == self._search_api.category_value:
                if category_value == self._search_api.category_value:
                    self._url_factory.adjust_by_search_api(self._search_api)
                else:
                    self._search_api.category_value = category_value
                    self._search_api.sub_category_value = ''
                    self._url_factory.adjust_by_search_api(self._search_api)
                url = self._url_factory.get_url_with_search_string(search_string)
    ...
    def __get_found_number_for_product_category_url__(self, url: str):
        # print('__get_found_number_for_product_category_url__: {}'.format(url))
        request = requests.get(url)
        tree = html.fromstring(request.content)
        xpath_numbers = tree.xpath('//div[@class="{}"]'.format(SCLS.FOUND_NUMBERS))
