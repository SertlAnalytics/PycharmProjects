    ...
    def __are_sales_similar__(self) -> tuple:
        ...
        for label in similar_label_list:
            if label == EL.ANIMAL:
                ...
                return is_identical_or_similar_dict[EL.PRODUCT] or is_identical_or_similar_dict[EL.OBJECT], label
            elif label == EL.PRODUCT:
                return is_identical_or_similar_dict[EL.COMPANY] or is_identical_or_similar_dict[EL.OBJECT], label
            elif label in [EL.TARGET_GROUP, EL.COLOR]:
                return is_identical_or_similar_dict[EL.OBJECT], label
            else:
                return True, label
        return False, ''
