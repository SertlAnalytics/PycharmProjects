...
from sertl_analytics.constants.pattern_constants import PRED, STBL, MP
...
class PatternPredictorOptimizer:
    def __init__(self, db_stock: StockDatabase):
        ...
        self._class_dict = {MP.MODEL_NAME: [], MP.TABLE: [], MP.PREDICTOR: [], MP.LABEL: [],
    ...
    def predict(self, table_name: str, predictor: str, label: str, x_data: list):
        ...
        return self.__get_optimal_prediction__(table_name, predictor, label, x_data_array)

    def __get_optimal_prediction__(self, table_name, predictor, label, x_data: np.array):
    ...
    @staticmethod
    def __get_key_for_class_key_list__(table_name: str, predictor: str, label: str):
        return '{}-{}-{}'.format(table_name, predictor, label)
    ...
    def __calculate_class_metrics_for_predictor_and_label__(self, table_name: str, predictor: str, label: str):
        key = self.__get_key_for_class_key_list__(table_name, predictor, label)
        # print('__calculate_class_metrics_for_predictor_and_label__: key={} ({})'.format(key, self._class_key_list))
        if key not in self._class_key_list:
            self.__calculate_class_metrics__(table_name, predictor, label)
            self._class_key_list.append(key)

    def __check_existing_trained_models__(self, model_name: str, table_name: str, predictor: str, label: str):
        key = self.__get_key_for_trained_model__(model_name, table_name, predictor, label)
        if key not in self._trained_model_dict:
            self._trained_model_dict[key] = self.__get_trained_model__(model_name, table_name, predictor, label)

    def __get_precision_for_model_value__(self, model_name: str, table_name: str, predictor: str, label: str, value):
        self.__calculate_class_metrics_for_predictor_and_label__(table_name, predictor, label)
        df = self._df_metric
    ...
    def get_metrics_for_model_and_label_as_data_frame(
            self, model_name: str, table_name: str, predictor: str, label: str):
        self.__calculate_class_metrics_for_predictor_and_label__(table_name, predictor, label)
        df = self._df_metric
    ...
    def get_sorted_value_list_for_predictor_label(self, model_name: str, table_name: str, predictor: str, label: str):
        self.__calculate_class_metrics_for_predictor_and_label__(table_name, predictor, label)
        df = self._df_metric
        df_filtered = df[np.logical_and(df[MP.MODEL_NAME] == model_name,
                                        np.logical_and(df[MP.TABLE] == table_name,
    ...
    def get_metric_value_list_for_predictor_label(self, model_name: str, table_name: str,
                                                  predictor: str, label: str, metric: str):
        df = self._df_metric
        df_filtered = df[np.logical_and(df[MP.MODEL_NAME] == model_name,
                                        np.logical_and(df[MP.TABLE] == table_name,
                                                       np.logical_and(df[MP.PREDICTOR] == predictor,
    ...
    def __calculate_class_metrics__(self, table_name: str, predictor: str, label: str):
        ...
        for model_name, model in self._model_dict.items():
            self.__print_class_metrics_details__(model_name, table_name, predictor, label, )
        ...
        self._df_metric = pd.DataFrame.from_dict(self._class_dict)
        self._df_metric = self._df_metric[[key for key in self._class_dict]]
        # print('df_metric.shape={}, df_metric.head={}'.format(self._df_metric.shape, self._df_metric.head()))

    @staticmethod
    def __print_class_metrics_details__(model_name: str, table: str, predictor: str, label: str):
        key_model = '{}-{}-{}-{}'.format(model_name, table, predictor, label)
