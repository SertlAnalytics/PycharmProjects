...
class TradeEnvironment(EnvironmentInterface):
    ...
    def init_by_entity(self, trade_entity: TradeEntity):
        self.__init_by_entity__(trade_entity)

    def __init_by_entity__(self, trade_entity: TradeEntity):
        self._done = False
        self._trade_entity = self._trade_entity_collection.get_first_element() if trade_entity is None else trade_entity
    ...
    def __get_entity_by_random__(self) -> TradeEntity:
        random_index = random.randint(1, self._trade_entity_collection.elements)
        return self._trade_entity_collection.get_nth_element(random_index)
    ...
    def __adjust_new_wave_tick_to_observation__(self):
        ...
        obs_new.current_tick_pct = round(self._step_counter/self.max_steps * 100, 2)
    ...
    def __calculate_end_with_respect_to_new_wave_tick__(self):
        if self._step_counter == self._max_steps:
            ...
            self._step.reward = self.observation_last.stop_loss_pct
            self._done = True
